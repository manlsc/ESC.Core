    ...
    var grid = List.SimpleList.extend({
        createDom: function () {
            var _self = this,
                ...
                width = _self.get("width");
            if (!width) {
                var totalWidth = _self._getColumnsWidth();
                _self.set("width", totalWidth);
            } else if (width) {
        ...
        getFitHeight: function () {
            var _self = this,
                appendheigh = 0,
                parent = _self.get('el').parent(),
                docHeigh = parent.height(),
                siblings = parent.siblings();
            BUI.each(siblings,
                function (elem) {
                    var node = $(elem);
                    if (node.css("position") !== "absolute") {
                        appendheigh += node.outerHeight();
                    }
                });
            _self.set('heigh', docHeigh - appendheigh);
        },
        onAdd: function (e) {
            var _self = this,
                store = _self.get('store'),
