...
namespace ESC.Infrastructure.Repository
{
    ...
    /// 采购入库 +
    /// </summary>
    public class WPurchaseRepository : BaseRepository<WPurchase>
    {

        #region 构造

        public WPurchaseRepository() : base(){}

        public WPurchaseRepository(string connectionString) : base(connectionString){}

        public WPurchaseRepository(DatabaseContext dbContext) : base(dbContext){}
        ...
        /// <returns></returns>
        public int ApprovePurchase(WPurchase purchase)
        {
            string sql = string.Format("UPDATE WPurchase SET UpdateDate='{0}',UpdateBy={1},StockStatus={2} WHERE ID={3} AND StockStatus={4}", purchase.UpdateDate.ToString("yyyy-MM-dd HH:mm:ss"), purchase.UpdateBy, StockStatusEnum.Approve, purchase.ID, StockStatusEnum.New);
            return Execute(sql);
        }
        ...
        /// <returns></returns>
        public int RemovePurchaseByStatus(int otherId, int stockStatus)
        {
            string sql = "DELETE FROM WPurchase WHERE ID=" + otherId + " AND StockStatus=" + stockStatus;
            return Execute(sql);
        }
    }
}
